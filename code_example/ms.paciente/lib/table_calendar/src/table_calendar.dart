...
class TableCalendar<T> extends StatefulWidget {
  ...
  final Future<List<T>> Function(DateTime day)? eventLoader;
...
class _TableCalendarState<T> extends State<TableCalendar<T>> {
  ...
  Widget _buildCell(DateTime day, DateTime focusedDay) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (!isDisabled) {
          final futwidget = build_cell_event(
              day, widget.eventLoader, constraints, shorterSide);
          if (futwidget != null) {
            children.add(futwidget);
          }
        }
  ...
  Widget? build_cell_event(
      day,
      Future<List<T>> Function(DateTime day)? eventLoader,
      constraints,
      shorterSide) {
    final _logger = getLogger('CalendarViewModel');
    final Future<List<T>>? events = eventLoader?.call(day) ?? Future(() => []);

    var fut_widget = FutureBuilder<List<T>>(
      future: events,
      builder: (BuildContext context, AsyncSnapshot<List<T>> snapshot) {
        ...
        if (snapshot.hasData) {
          ...
          Widget? markerWidget = widget.calendarBuilders.markerBuilder
              ?.call(context, day, snapshot.data!);

          if (snapshot.data!.isNotEmpty && markerWidget == null) {
          ...
          if (markerWidget != null)
            //children = <Widget>[markerWidget];
            child = markerWidget;
          else
            child = Column();
          ;
        } else if (snapshot.hasError) {
          _logger.d("Has Data Error");
          /*children = <Widget>[
            Icon(
              Icons.error_outline,
              color: Colors.red,
